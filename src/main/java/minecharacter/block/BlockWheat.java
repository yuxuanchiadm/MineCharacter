...
public class BlockWheat extends Block {
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		List list = MineCharacter.proxy.findTarget(par1World, AxisAlignedBB
				.getBoundingBox(par2 - 5.0D, par3 - 5.0, par4 - 5.0D,
						par2 + 5.0D, par3 + 5.0, par4 + 5.0), null);
	...
	private void pp(EntityAnimal fAnimal, Entity entity, World world) {
		...
		for (int i = 0; i < 7; ++i) {
			...
			world.spawnParticle(
					"heart",
					fAnimal.posX
							+ (double) (this.ran.nextFloat() * fAnimal.width * 2.0F)
							- (double) fAnimal.width,
					fAnimal.posY + 0.5D
							+ (double) (this.ran.nextFloat() * fAnimal.height),
					fAnimal.posZ
							+ (double) (this.ran.nextFloat() * fAnimal.width * 2.0F)
							- (double) fAnimal.width, d0, d1, d2);
			world.spawnParticle(
					"heart",
					sAnimal.posX
							+ (double) (this.ran.nextFloat() * sAnimal.width * 2.0F)
							- (double) sAnimal.width,
					sAnimal.posY + 0.5D
							+ (double) (this.ran.nextFloat() * sAnimal.height),
					sAnimal.posZ
							+ (double) (this.ran.nextFloat() * sAnimal.width * 2.0F)
							- (double) sAnimal.width, d0, d1, d2);
		}
		this.time--;
