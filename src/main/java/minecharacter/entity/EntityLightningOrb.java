		...
		if (this.lightningState >= 0) {
			if (this.worldObj.isRemote) {
				...
				List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(
						this,
						AxisAlignedBB.getAABBPool().getAABB(this.posX - d0,
								this.posY - d0, this.posZ - d0, this.posX + d0,
								this.posY + 6.0D + d0, this.posZ + d0));
				...
				for (int l = 0; l < list.size(); ++l) {
					...
					if (entity instanceof EntityItem) {
						if (((EntityItem) entity).getEntityItem().getItem().equals(
							...
							this.worldObj.spawnEntityInWorld(new EntityItem(
									worldObj, this.posX, this.posY, this.posZ,
									new ItemStack(InitItem.assassinSteel,
											((EntityItem) entity)
													.getEntityItem().stackSize,
											0)));
							entity.setDead();
